pfUI:RegisterModule("firstrun", "vanilla:tbc", function ()
  ...
  pfUI.firstrun:AddStep("profile", function()
    local f = CreateFirstRunPage()
    f.text:SetText(T["Welcome to |cff33ffccShaw|cffffffffUI|r!\n\nClick below to load the ShawUI profile, or hit \"Defaults\" to continue with default settings."])

    f.ShawUI = CreateFrame("Button", nil, f, "UIPanelButtonTemplate")
    f.ShawUI:SetWidth(160)
    f.ShawUI:SetHeight(25)
    f.ShawUI:SetPoint("BOTTOM", 0, 90)
    f.ShawUI:SetTextColor(1,1,1)
    f.ShawUI:SetText("|cff33ffccShaw|cffffffffUI")
    f.ShawUI:SetScript("OnClick", function()
      _G["pfUI_config"] = CopyTable(pfUI_profiles["ShawUI"])
      pfUI_init.selected_profile = "ShawUI"
      pfUI:LoadConfig()
      ReloadUI()
    end)
    SkinButton(f.ShawUI)
